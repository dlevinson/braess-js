...
C(q)=A-Bq.
\]
The default elastic case sets \(A=180\) and \(B=30\), so the zero-cost demand intercept remains \(A/B=6\). For each trial \(q\), the model solves fixed-demand equilibrium and compares equilibrium cost with \(A-Bq\). A bisection loop finds the quantity where willingness to pay equals equilibrium cost.
