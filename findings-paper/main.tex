...
For each scenario, the model enumerates all simple origin-destination paths and solves static Wardrop user equilibrium by Method of Successive Averages \citep{sheffi1985urban}. At each iteration, link costs are evaluated from current flows, all demand is assigned to the current shortest path, and path flows are averaged using the diminishing step size \(1/k\). This averaging is the convergence device: it damps path switching. The stopping tolerance is \(10^{-6}\) after at least five iterations, with a 120-iteration maximum. For the nonnegative affine and Bureau of Public Roads-style separable link costs used here, this is the standard monotone user-equilibrium setting.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \draw[arrow] (compare.south) -- ++(0,-0.42) -| node[pos=0.22, below, font=\scriptsize] {no} (adjust.south);
