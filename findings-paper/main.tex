...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[demand, below=1.05cm of wardrop] (inverse) {Inverse demand\\\(C(q)=A-Bq\)};
    \node[equilibrium, right=of inverse] (equilibrium) {Equilibrium\\\(c(q^*)=C(q^*)\)};
    \node[adjust, left=of inverse] (adjust) {Adjust \(q\)\\if costs differ};

    \draw[arrow] (network) -- (trial);
    \draw[arrow] (trial) -- (wardrop);
    \draw[arrow] (wardrop) -- (cost);
    \draw[arrow] (cost) -- node[right, font=\scriptsize] {compare} (equilibrium);
    \draw[arrow] (inverse) -- (equilibrium);
    \draw[arrow] (inverse) -- (adjust);
